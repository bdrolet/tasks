...
| **Enrichment** | Claude via `clients/claude.py` (Haiku summary, Sonnet deadline extraction) — `ANTHROPIC_API_KEY`, mounted on **both** CFs: the events CF enriches, the webhook CF condenses digest bullets |
...
| **API** | `tasks-api` — Cloud Run FastAPI service (`api/`), search/fetch/add/update for tasks + comments; list/create projects, list tags, subtasks; auth is Cloud Run IAM — `roles/run.invoker` per caller in `terraform/api.tf`; callers send `gcloud auth print-identity-token`; image in AR repo `tasks`, deployed by `deploy-api.yml`; `tasks-api.drolet.cloud` |
